...
void drawUI() {
    ...
    // Draw instructions
    mvprintw(3, 2, "Use arrow keys or j/k to navigate, Enter to select, q to quit");
    ...
    // Draw hostnames
    int visible_items = (y - list_y - 2 > LIST_HEIGHT) ? LIST_HEIGHT : y - list_y - 2;
    visible_items = (visible_items < hostname_count - scroll_offset) ? visible_items : hostname_count - scroll_offset;
    ...
    mvhline(y - 2, 1, ' ', x - 2);
    if (hostname_count > 0) {
        mvprintw(y - 2, 2, "Selected: %s (Press Enter to confirm)", hostnames[selected_index]);
    } else {
